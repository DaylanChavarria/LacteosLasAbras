...
--
-- Servidor: 50.62.209.121:3306
-- Tiempo de generación: 08-03-2017 a las 21:46:56
...
CREATE DEFINER=`lasabras17`@`%` PROCEDURE `ActualizarGaleria` (IN `atNombre_` TEXT, IN `atDescripcion_` TEXT, IN `atIdioma_` INT, IN `atcodigoImagen_` INT)  NO SQL
...
CREATE DEFINER=`lasabras17`@`%` PROCEDURE `ActualizarProducto` (IN `atNombre_` TEXT, IN `atPrecio_` INT, IN `atDescripcion_` TEXT, IN `atIdioma_` INT, IN `atcodigoProducto_` INT, IN `atNombreImagen_` TEXT)  NO SQL
...
CREATE DEFINER=`lasabras17`@`%` PROCEDURE `EliminarImagenDeInicio` (IN `atId_` INT)  NO SQL
DELETE FROM `la_imagen` WHERE atId=atId_$$

CREATE DEFINER=`lasabras17`@`%` PROCEDURE `EliminarImagenGaleria` (IN `atId_` INT)  NO SQL
delete from la_galeria where atcodigoImagen = atId_$$

CREATE DEFINER=`lasabras17`@`%` PROCEDURE `EliminarProducto` (IN `atId_` INT)  NO SQL
delete from la_productos where atcodigoProducto=atId_$$

CREATE DEFINER=`lasabras17`@`%` PROCEDURE `IngresarImagenDeInicio` (IN `atNombre_` TEXT, IN `atTipo_` INT)  NO SQL
INSERT INTO `la_imagen` VALUES(NULL, atNombre_,atTipo_)$$

CREATE DEFINER=`lasabras17`@`%` PROCEDURE `IngresarImagenEnGaleria` (IN `atNombre_` TEXT, IN `atDescripcion_` TEXT, IN `atTipo_` INT, IN `atCodigoImagen_` INT)  NO SQL
INSERT INTO `la_galeria` VALUES(NULL, atNombre_,atDescripcion_,atTipo_,atCodigoImagen_)$$

CREATE DEFINER=`lasabras17`@`%` PROCEDURE `InsertarProducto` (IN `atId_` INT, IN `atNombre_` TEXT, IN `atPrecio_` INT, IN `atDescripcion_` TEXT, IN `atIdioma_` INT, IN `codigoProducto_` INT, IN `atNombreImagen` TEXT)  NO SQL
INSERT INTO la_productos VALUES (NULL, atNombre_, atPrecio_, atDescripcion_, atIdioma_,codigoProducto_,atNombreImagen)$$

CREATE DEFINER=`lasabras17`@`%` PROCEDURE `ObtenerImagenesInicio` (IN `atTipo_` INT)  NO SQL
select * from la_imagen where atTipo = atTipo_$$

CREATE DEFINER=`lasabras17`@`%` PROCEDURE `ObtenerTodosGaleria` (IN `atIdioma_` INT)  NO SQL
IF(atIdioma_ = -1)THEN
...
CREATE DEFINER=`lasabras17`@`%` PROCEDURE `ObtenerTodosProductos` (IN `atIdioma_` INT)  NO SQL
IF(atIdioma_ = -1)THEN
...
(31, 'gal7.png', 'Nuestra leche es sometida a altos estándares de calidad e inocuidad. Una vez que finaliza el proceso de ordeño mecánico, la leche se deja enfriar a temperatura ambiente, para luego ser refrigerada inmediatamente para mantener su frescura y sanidad.', 0, 4),
(32, 'gal7.png', 'Our milk is subject to high standards of quality and safety. Once the mechanical milking process is completed, the milk is allowed to cool to room temperature, then cooled immediately to maintain its freshness and sanity.', 1, 4),
(39, 'queso.JPG', '...', 0, 5),
(40, 'queso.JPG', '...', 1, 5);
...
(6, 'Queso Palmito', 0, 'El delicioso queso palmito es una de nuestras especialidades. Su textura y sabor son únicos.', 0, 2, 'palmito.png'),
(7, 'Palmito cheese', 0, 'The delicious palmito cheese is one of our specialties. Its texture and taste are unique.', 1, 2, 'palmito.png'),
(8, 'Queso Mozzarella', 0, 'El uso mayoritario del Queso Mozzarella, y el que le ha dado fama, es para la fabricacion de pizzas', 0, 3, 'mozzarella2.png'),
(9, 'Mozzarella Cheese', 0, 'The majority use Mozzarella Cheese, and that has made him famous, is for making pizzas', 1, 3, 'mozzarella2.png'),
(10, 'Queso Turrialba', 0, 'Nuestro delicioso Queso Turrialba cuenta con la certificación de denominación de origen.', 0, 4, 'turrialba.png'),
(11, 'Turrialba Cheese', 0, 'Our delicious Turrialba Cheese has the certification of denomination of origin.', 1, 4, 'turrialba.png'),
(12, 'Queso con chile', 0, 'El Queso con chile es una nueva alternativa para aquellos amantes del picante y para aquellos que desean provar nuevos sabores.', 0, 5, 'chile.png'),
(13, 'Cheese with chile', 0, 'El Queso con chile is a new alternative for those who love spicy and for those who want to taste new flavors.', 1, 5, 'chile.png');
...
--
ALTER TABLE `la_galeria`
  MODIFY `atId` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=41;
--
-- AUTO_INCREMENT de la tabla `la_imagen`
--
ALTER TABLE `la_imagen`
  MODIFY `atId` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=18;
--
-- AUTO_INCREMENT de la tabla `la_productos`
--
ALTER TABLE `la_productos`
  MODIFY `atId` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
--
